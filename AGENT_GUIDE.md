...
Hashing now copies a SHA-256 state containing the fixed seed prefix, preserving the exact first successful nonce. Startup measures 150,000 attempts per trial through the actual execution loop, avoiding calibration based on a lucky search. See the [hash proof](HASH_PROOF.md). Monte Carlo now batches Python's exact random draws through NumPy float64 arithmetic when installed, with a calibrated Python fallback. See the [Monte Carlo proof](MONTE_CARLO_PROOF.md). Sorting uses exact rejection sampling, optional NumPy sorting with bounded integer subtotals, and one final modular reduction. Its startup timing model measures the selected backend; see the [sort proof](SORT_NUMPY_PROOF.md). The benchmark number reported during registration still comes from the SDK's standard calibration; the fast executors have additional timing models inside our agent.
